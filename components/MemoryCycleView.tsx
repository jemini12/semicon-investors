...
export default function MemoryCycleView() {
    ...
    const [data, setData] = useState<{ [symbol: string]: { price: number; change: number } }>({});
        ...
        <div className="w-full p-6 bg-portal-gray/30 border border-white/5 backdrop-blur-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {memoryPeers.map(item => {
                    ...
                    return (
                        <div key={item.symbol} className="bg-portal-black/50 p-4 border-t-2 border-white/10 hover:border-portal-accent transition-colors">
                            ...
                            <div className={`flex items-center mt-2 text-sm font-mono font-bold ${isNeutral ? 'text-slate-500' : isPositive ? 'text-green-500' : 'text-red-500'}`}>
